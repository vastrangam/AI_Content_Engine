...
|---|---:|---:|
| Start here | 2 | — |
| The delivered documents | 25 | 1019 KB |
...
| The document generators and the archive builders | 69 | 4.4 MB |
| The earlier prototype app line | 184 | 10.0 MB |
| Logos, fonts and the brand sheet | 16 | 1.4 MB |
| Deployment, tooling and the build | 21 | 355 KB |
...
**25 files · 1019 KB.** The reading matter — the plan, the design argued, the runbook, the rulebook, the audits, and this contents list. Each .md has a .pdf beside it rendered from the same source, so the two cannot disagree.
...
| `CURRENT_STATE_AUDIT.md` | What is actually in this repository, counted at the moment this file was generated. | 12 KB |
...
| `MEDHAVA_BUILD_GUIDE.md` | How this platform is designed and built. | 191 KB |
...
| `MEDHAVA_START_HERE.md` | Paste this whole file into a new conversation before anything else. | 6 KB |
...
**66 files · 1.3 MB.** Modules, apps, rules, tools, the stack, the requirements registry, and the checkers that gate them. No count in any document here is typed: it is read from one of these files, which is why the counts have changed twice without a document going stale.

| File | What it says it is | Size |
|---|---|---:|
| `brand/site/INDEX.md` | A unified ERP: 30 modules and 165 apps over one shared data core. | 74 KB |
...
| `brand/site/backlog.js` | THE BACKLOG — the lines of the master specification this product does not name at all. | 9 KB |
...
| `brand/site/checkbacklog.js` | THE BACKLOG, CHECKED AGAINST THE SHEET THAT PRODUCED IT. | 10 KB |
| `brand/site/checkcompetitor.js` | NOTHING MEDHAVA SHIPS NAMES A COMPETITOR. | 15 KB |
...
| `brand/site/checkedition.js` | NO MEDHAVA-NAMED ARTEFACT MAY CARRY A TRADE WORD. | 10 KB |
...
| `brand/site/checkprivacy.js` | NOBODY REAL IS NAMED IN A TRACKED FILE, AND NO TRACKED FILE PAIRS A PERSON WITH MONEY. | 18 KB |
...
| `brand/site/llms.txt` | *no description in the file itself* | 7 KB |
...
| `brand/site/modules.js` | THE canonical list. The website, every PDF, INDEX.md, llms.txt and the structural audit all | 69 KB |
...
| `brand/site/rules.js` | THE RULEBOOK — the canonical list, in the same spirit as modules.js. | 111 KB |
...
| `brand/site/shots.js` | ONE LIVE-LOOKING SCREEN PER MODULE — the neutral edition. | 39 KB |
...
**69 files · 4.4 MB.** Every delivered document is written by a script in here, from the registers above. To change a document you change its generator or its register, never the document — a hand-edit is overwritten the next time anything is built.
...
| `brand/delivery/website/MEDHAVA_BOS/shots/m14.png` | *binary — carries no readable header* | 95 KB |
| `brand/delivery/website/MEDHAVA_BOS/shots/m15.png` | *binary — carries no readable header* | 95 KB |
...
| `brand/delivery/website/mkstarthere.js` | MEDHAVA_START_HERE.md — THE ONE PROMPT THAT WORKS WITH NOTHING ELSE IN HAND. | 13 KB |
...
| `brand/suite/deep/shots_m04.js` | HD screenshots of every screen of Module 04, in populated states, for the PDF books. | 8 KB |
...
**21 files · 355 KB.** The service unit, the web-server blocks, the PDF renderer, the evidence log and the checks that run on every push. Nothing here is business logic; all of it is how the business logic gets onto a machine and stays honest.
...
| `.gitignore` | Regenerable build intermediates — rebuild with the scripts in the same folder. | 7 KB |
| `MASTER_SPEC_COVERAGE.xlsx` | *binary — carries no readable header* | 61 KB |
...
| `docs/verification/EVIDENCE.md` | Every entry below is one command that was actually run, with the exit code the process returned, the revision it ran against, and the SHA-256 of the files it was about. | 61 KB |
...
| `tools/evidence.js` | EVIDENCE CAPTURE — the record that does not depend on my word. | 13 KB |
| `tools/evidence.test.js` | THE EVIDENCE TOOL, TESTED — because a check that only holds while somebody remembers to run it is not a check. | 12 KB |
...
- `brand/site/llms.txt` — 7 KB, no description in the file itself
