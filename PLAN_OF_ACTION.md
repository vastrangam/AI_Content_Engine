...
| 16 | HR & Payroll | 6 | 0 | 0 | 6 |
| 17 | Marketing | 9 | 0 | 0 | 9 |
| 18 | AI Content Engine | 8 | 0 | 1 | 7 |
| 19 | SEO, AEO & AIO | 3 | 0 | 0 | 3 |
| 20 | Projects & Collaboration | 7 | 0 | 0 | 7 |
| 21 | Dashboard & BI | 9 | 3 | 0 | 6 |
| 22 | AI Assistant, Agents & Automation | 7 | 0 | 0 | 7 |
| 23 | Developer Platform | 9 | 0 | 0 | 9 |
| 24 | Agile & Sprints | 4 | 0 | 0 | 4 |
| 25 | Meetings | 4 | 0 | 0 | 4 |
| 26 | Calendar | 4 | 0 | 0 | 4 |
| 27 | Mail | 5 | 0 | 0 | 5 |
| 28 | Documents | 5 | 0 | 0 | 5 |
| 29 | Learning | 3 | 0 | 0 | 3 |
| 30 | Integration Platform | 3 | 0 | 0 | 3 |
| 31 | Identity & IT | 4 | 0 | 0 | 4 |
| | **Total** | **165** | **16** | **2** | **147** |
...
**302 rules across 31 modules. 89 of them are enforced by a test that runs
...
| 23 · Developer Platform | 1 | 0 | 1 |
| 24 · Agile & Sprints | 1 | 0 | 1 |
| 25 · Meetings | 1 | 0 | 1 |
| 26 · Calendar | 1 | 0 | 1 |
| 27 · Mail | 1 | 0 | 1 |
| 28 · Documents | 1 | 0 | 1 |
| 29 · Learning | 1 | 0 | 1 |
| 30 · Integration Platform | 1 | 0 | 1 |
| 31 · Identity & IT | 1 | 0 | 1 |
| **Total** | **302** | **89** | **213** |
...
| R02.2 | **Every version of a specification is kept** | a sample round changes a measurement, a fabric or a trim | a new version is written and the old one stays readable | editing the specification in place — a worker paid against last month’s spec must still be able to show what it said | SPECIFIED |
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R08.1 | **Sets are pooled across every maker before the minimum is taken** | completed sets are counted for a design | every maker’s pieces for that design are pooled first, and the set count is the minimum across the populated member columns of the pool | counting sets per maker row and adding them up, which loses every set completed by two people between them | **ENFORCED** · brand/suite/studio/verify_studio.js · pooling happens before the minimum, not per maker row |
| R08.2 | **A surplus piece is paid for, and is not a set** | a maker produces more of one component than the set needs | the extra is named individually and paid at its own piece rate | adding it to the set count, and never leaving it unpaid because it did not complete a set — the person made it either way | **ENFORCED** · brand/suite/studio/verify_studio.js · a surplus piece is named, is still paid for, and is never added to the sets |
| R08.3 | **A design counts on the components it actually has** | a design is made of fewer component types than the usual set | it is counted on the members it does have | returning zero because an optional member is absent, which silently unpays a whole design | **ENFORCED** · brand/suite/studio/verify_studio.js · a single-component design counts on what it has, not zero |
| R08.4 | **A missing rate posts zero and is flagged, never guessed** | a design has no entry in the piece-rate master | it costs zero and the design is named in the summary | inferring a rate from a similar design — a guessed rate is a wrong payment to a real person | **ENFORCED** · brand/suite/studio/verify_studio.js · a missing rate posts zero and is flagged, never guessed |
| R08.5 | **A two-row heading is read as two rows** | the production grid uses a merged heading over component columns | both header rows are read so repeated component names stay distinct columns | reading only the first row, which collapses three same-named component columns into one and undercounts the work | **ENFORCED** · brand/suite/studio/verify_studio.js · the two-row heading is read, so three same-named columns stay three components |
| R08.6 | **A worker written as a pair stays one unit** | two names share one row as a working pair | they are treated as a single paying unit | splitting them into two workers, which halves each person’s recorded output and breaks the payout | **ENFORCED** · brand/suite/studio/verify_studio.js · a maker written as a pair stays one unit |
...
| R08.14 | **An advance to a worker is a balance, not a deduction from nowhere** | an advance is paid | it is held against that worker and recovered from later payouts, with the running balance visible | deducting an amount at payout time that cannot be traced to a specific advance | SPECIFIED |
...
| R08.17 | **A set type comes from the rate master, and an inferred one says so** | a design is classified into a set type | the rate master’s Set column decides it; when the design is absent, the type is inferred from which component columns actually carry pieces and the design is flagged as inferred | presenting an inferred classification as though it came from the master | **ENFORCED** · brand/suite/studio/verify_studio.js · the two-row heading is read, so three same-named columns stay three components |
| R08.18 | **An alteration caused by the worker’s own mistake is unpaid** | a piece is reworked because of an error by the person who made it | the alteration hours are recorded and paid at zero | paying for the rework at the standard alteration rate, and never leaving the hours unrecorded — the time still happened and the design still bore the cost | SPECIFIED |
| R08.19 | **Alteration time is paid at the alteration rate, not the piece rate** | admin-assigned alteration hours are settled | they are paid at the hourly alteration rate in force and added to that worker’s payout | folding alteration hours into the piece count, which corrupts both the production figure and the earnings figure at once | SPECIFIED |
...
| R16.8 | **Staff and piece-rate workers sit in one register** | payroll is prepared | monthly staff and piece-rate workers are computed in the same run and paid from the same register | running two payrolls that have to be added together by hand | SPECIFIED |
...
**What it is.** One decision, not twenty-five: whether anything outside this system may call
it. Twenty-five lines of the specification are that single capability written out from four
different places. A versioned surface with keys and rate limits, webhooks going out, functions
on top, and only then a builder that assembles a screen without code — in that order, because a
builder over nothing is a demonstration.

<!-- RULES:23 -->

**The rulebook — 1 rules, 0 enforced by a test that runs today**

| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R23.1 | **A published interface is versioned before it has a second caller** | anything outside this system is given a way to call in | the surface carries a version, and an existing version keeps its response shape until it is retired on an announced date | changing what an existing caller receives without a version change, which breaks somebody else’s software at a moment nobody chose | SPECIFIED |

<!-- /RULES:23 -->

**Reads** ← every module · **Writes** → every module

**Done when.** An outside caller can read a list through a versioned surface using a key scoped
to what its holder may already see, a webhook delivery is signed and retried, and neither can
reach a record the key's owner could not open directly.
...
**What it is.** Thirteen lines that amount to one board, one boundary and one chart. Beside
Projects & Collaboration rather than inside it: a task is done or not, a sprint asks how much a
team reliably finishes in a fixed period and holds that against the last one. A business running
a factory may never want it, which is why it can be switched off entirely.
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R24.1 | **A sprint figure is read from the board, never typed** | velocity, burndown or a completion figure is shown | it is computed from the work items themselves and their recorded state changes | a progress number somebody enters by hand, because a typed figure is a figure that can be rounded | SPECIFIED |

<!-- /RULES:24 -->

**Reads** ← Projects & Collaboration · HR & Payroll · **Writes** → Projects & Collaboration

**Done when.** Velocity and burndown are computed from the work items and their recorded state
changes, no figure on the board can be typed by hand, and work added mid-sprint shows as added.
...
**What it is.** The half of meetings that is not live video. Video needs media servers and
bandwidth, which a business rents — that half is a constraint. This half needs none of it, and it
is the half with lasting value: six months later nobody wants the recording, they want to know
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R25.1 | **What was decided outlives the call it was decided in** | a meeting ends | the attendance, the decisions and any poll result remain attached to the record the meeting was about | holding the only copy of an outcome inside a session that closes, which is how a decision becomes a disagreement six weeks later | SPECIFIED |

<!-- /RULES:25 -->

**Reads** ← CRM · Calendar · HR & Payroll · **Writes** → CRM · Calendar

**Done when.** A meeting's attendance, decisions and poll results survive the session and are
attached to the customer or project record they concerned.
...
**What it is.** Eleven lines that are genuinely one thing. Recurring events and time zones are
where the difficulty sits, and both are library problems rather than judgement calls. Syncing to
an outside calendar needs that provider's credentials and is a constraint; a calendar of this
system's own is not.
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R26.1 | **Every moment is stored absolute and displayed local** | an event is saved, moved or shown | it is held as an absolute instant and rendered in each reader’s own zone | storing a wall-clock time without its zone, which silently moves a meeting when an offset changes between the booking and the day | SPECIFIED |

<!-- /RULES:26 -->

**Reads** ← every module · **Writes** → Meetings · Projects & Collaboration · HR & Payroll

**Done when.** One occurrence of a repeating event can be moved without disturbing the series,
and an event booked across two zones still means the same instant to both after an offset change.
...
**What it is.** The reading and organising half of email. Hosting a mail domain is a constraint —
deliverability is earned over months — but once a mailbox exists, filtering and routing it is
ordinary work. Separate from the Helpdesk on purpose: a ticket has an owner and a resolution, a
mailbox is a stream, and treating the stream as a queue is how real questions get closed unread.
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R27.1 | **A message that was filed automatically says so on its face** | a filter or rule moves, tags or forwards an incoming message | the rule that fired is recorded on the message and visible to whoever opens it | a silent redirection, because a mail that vanished into a folder is indistinguishable from one that was never sent | SPECIFIED |

<!-- /RULES:27 -->

**Reads** ← CRM · every module · **Writes** → CRM

**Done when.** Every message that a rule moved, tagged or forwarded says which rule did it, and a
message can become a Helpdesk ticket with its thread attached.
...
**What it is.** A word processor, a presentation tool and notes — eight lines that are three
substantial products listed as though they were features. Honestly the worst effort-to-value
ratio in the design: every business already has these and nobody adopts a business system for its
word processor. It is named so the specification is not silent about it, and ranked accordingly.
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R28.1 | **A document belongs to the record it is about** | a document, presentation or note is created here | it is filed against the order, party, case or project it concerns and is found by that record | a document whose only address is a folder somebody has to remember, which is the filing cabinet this module exists to replace | SPECIFIED |

<!-- /RULES:28 -->

**Reads** ← CRM · Projects & Collaboration · **Writes** → CRM

**Done when.** A document is reachable from the order, party or project it is about rather than
from a folder somebody has to remember, and a previous version can be recovered.
...
**What it is.** Course material, the lessons in it, an assessment, and a record of who completed
what — one straightforward application over data HR & Payroll already holds. The one module in
this group with an obvious near-term case: on a factory floor a process has to be taught, and the
record of who was taught it is what a compliance audit asks for.
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R29.1 | **Completion is a dated record against a named person** | somebody finishes a course or passes an assessment | the completion is recorded with the person, the material version and the date, and can be produced when an auditor asks | reporting that training was offered as though it were training completed, which is the distinction an inspection turns on | SPECIFIED |

<!-- /RULES:29 -->

**Reads** ← HR & Payroll · Quality & Compliance · **Writes** → HR & Payroll

**Done when.** A completion is a dated record against a named person and a material version, and
the report an inspector asks for can be produced from it.
...
**What it is.** The connector framework, deliberately separate from any connector. The framework
can be built and tested against recorded responses and needs nobody's credentials; the connectors
each need a live account and are constraints. It is only sensible after Module 23 exists.
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R30.1 | **A connector runs with the scope it was granted, not the scope it asks for** | an integration reads or writes through the framework | it acts within the permissions recorded when it was installed, and widening them is an approval with a record | a connector that quietly extends its own reach mid-run, which turns an integration into an unaudited second way in | SPECIFIED |

<!-- /RULES:30 -->

**Reads** ← every module · **Writes** → every module

**Done when.** A connector runs inside the scope recorded when it was installed, and widening
that scope is an approval with a record rather than something it can do to itself mid-run.
...
**What it is.** Four lines that are two different things, and the split matters. A second proof
at sign-in is small, standard and should exist before anyone signs in over the internet at all.
The other three are a product for IT departments, which a manufacturer is not — naming them
together keeps a genuine near-term need from queueing behind three things nobody asked for.
...
| # | The rule | When | Then | Never | State |
|---|---|---|---|---|---|
| R31.1 | **A second factor is required before the system is reachable from the internet** | sign-in is available anywhere outside a private network | a second proof is enforced for every account that can see money, staff or customer data | relying on a password alone on a public address, because one reused credential elsewhere then becomes a breach here | SPECIFIED |

<!-- /RULES:31 -->

**Reads** ← Platform · every module · **Writes** → Platform

**Done when.** No account that can see money, staff or customer data is reachable from a public
address with a password alone.
...
<!-- APPS -->
**31 modules · 165 apps.** The whole of what is being built,
