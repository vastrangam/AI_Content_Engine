...
**Building this trade’s own edition, from an empty folder to a running business.**
...
**This is a deployment guide.** Vastrangam BOS is not a different piece of software from
...
```bash
git tag -a archive-final -m "final state before the Medhava and Vastrangam projects"
...
`@vastrangam.com` email, the VPS exists to run things.
...
git config --global user.email "you@vastrangam.com"
```
...
**Where:** GitHub → New repository → name it `vastrangam-bos` → **Private** → no README, no .gitignore, no licence.
...
```bash
mkdir vastrangam-bos && cd vastrangam-bos
git init
...
```bash
# ARCHIVE is wherever you extracted the old project. Run from inside vastrangam-bos.
ARCHIVE=../old-project
...
git remote add origin git@github.com:<you>/vastrangam-bos.git
...
## Part 4 · Put vastrangam.com live
...
```bash
curl -sSI https://vastrangam.com | head -1
curl -sS  https://vastrangam.com | grep -c "Industry packs"
curl -sSI https://www.vastrangam.com | head -1
```
...
```bash
curl -sSI https://n8n.vastrangam.com | head -1
```
...
**Where:** `DEPLOYMENT.md` §2.4 — the systemd unit from `deploy/medhava-app.service`, nginx at `app.vastrangam.com`.

**Check it:**

```bash
curl -sSI https://app.vastrangam.com | head -1
```
...
| Ask & Print | `WORKS TODAY` | At an exhibition in Hyderabad, send one line from your phone: “ledger Kalamandir”, “print slips”. |
...
| App | State | What it is |
|---|---|---|
| PLM & Development | `SPEC` | Concept to a design that can actually be made: fabric and trim specification, sample rounds with the mill, costed trials against a target price, and sign-off — every version kept, so last season’s costing is still there. |
...
| App | State | What it is |
|---|---|---|
| Stock | `SPEC` | Live quantity by design, size and location, fabric in metres and pieces in numbers, with reorder alerts, lot tracking, set kits and dead-stock ageing. |
| Catalog / PIM | `SPEC` | One record per design — fabric, work, length, colour, size chart, images, HSN, MRP and what each panel actually sells it at — scored for Myntra and Amazon readiness before it lists. |
...
| App | State | What it is |
|---|---|---|
| CRM & Customer 360 | `WORKS TODAY` | Enquiry to confirmed order, then the full lifetime: what they bought, what came back, what they are worth and which new range to show them first. |
| Documents & eSign | `WORKS TODAY` | Mill agreements, job-work contracts, signed delivery challans, export documents and boutique credit terms filed against the party or order they belong to — found by that record, not by hunting through a folder. |
| Helpdesk & Live Chat | `WORKS TODAY` | A boutique asking where its parcel is, or a customer asking about a size — the question becomes a ticket tied to the order, with the whole history already open. |
...
| App | State | What it is |
|---|---|---|
| D2C Sales | `WORKS TODAY` | Orders from your own storefront, cart to dispatch, with loyalty and partial COD on a ₹4,400 anarkali. |
| B2B & Credit | `WORKS TODAY` | Boutique and chain orders on credit limits and tier pricing, with outstanding aged against each party’s own agreed terms. |
| Export | `WORKS TODAY` | Commercial invoice, packing list, LUT bond and IGST-refund tracking for the Gulf and UK buyers. |
| POS | `WORKS TODAY` | Counter billing at Udhna that draws on the same stock as the website — no second stock register. |
| Quotes & Proforma | `WORKS TODAY` | Send a quote, convert it to a confirmed order in one click. |
| Couriers & AWB | `SPEC` | Book the parcel on the order, compare couriers for that pin code, print the label with the design code on it, and follow the AWB to the door. |
...
| App | State | What it is |
|---|---|---|
| Procurement | `WORKS TODAY` | Enquiry to purchase order to goods receipt, with a strict three-way match: you ordered 100 metres, 100 arrived, quality accepted 96, and the bill is only cleared for 96. |
| Vendor Management | `WORKS TODAY` | Mill 360 — payables, ageing, a real risk score from accept rate and spend concentration, and sourcing that follows performance rather than habit. |
...
| App | State | What it is |
|---|---|---|
| Production Orders | `SPEC` | Cutting, stitching, embroidery, washing, finishing and checking — your own stages, with work-in-progress visible at each and nothing lost at the dyer. |
| Piece-rate & Contractors | `SPEC` | Karigars paid by the piece: pooled set completion, per-garment rates, alterations, rework and advances resolved into one payout. |
| BOM & Consumption | `SPEC` | What each design consumes — metres of fabric, zari, lining, buttons, packing — costed at today’s mill rates. |
| Maintenance | `SPEC` | Machines and the building: what is due for service, when it was last done, what it cost, and what stopped while it was down. |
...
| App | State | What it is |
|---|---|---|
| Quality Control | `SPEC` | Accept, reject or send for rework, with reasons that feed the mill’s accept rate and the karigar’s record. |
...
| App | State | What it is |
|---|---|---|
| Picking & Bins | `SPEC` | Pick lists in walking order through the godown, by design and size, so nobody crosses the floor twice. |
...
| Packing Video | `SPEC` | Every parcel filmed as it is packed and indexed by its order number, so when a panel says the wrong piece was sent, the clip goes into the claim. |
...
| COD Remittance | `SPEC` | What the courier collected at the door against what reached the Surat account, parcel by parcel, with every shortfall named and aged. |
| Handover & Manifest | `SPEC` | What is expected out today against what the pickup boy actually took, per courier and per service. |
...
| Invoicing | `SPEC` | GST tax invoices and receipts, worked out from the lines to the paise. |
...
| Finance Reports | `SPEC` | P&L, balance sheet, and profit by channel, design and SKU — so you know which anarkali actually earned money after commission, shipping and returns. |
...
| Fee & Commission Audit | `SPEC` | The commission a panel publishes for a category against what it actually took, style by style. |
| TCS & TDS Register | `SPEC` | Every rupee the panels deducted as TCS, and TDS on job work, matched against the portal’s own figures — so the credit you claim is the credit you are owed. |
...
| App | State | What it is |
|---|---|---|
| Marketplace OMS | `WORKS TODAY` | Myntra, Flipkart, Ajio, Amazon, Meesho, Nykaa and JioMart in a single queue — processed all together, channel-wise, or design-wise. |
...
| Manual Data Check | `SPEC` | The order and return sheets you already download from the panels, and the offline registers from the three shops — one file or a whole ZIP — read back as ten cross-checks: net sale after commission and fees, month, design, state, wrong returns, SPF claims, ads, payouts and GST. |
| Reconciliation | `SPEC` | Match every marketplace payout to the order line that earned it, and expose the gap. |
| Claims & Disputes | `SPEC` | Weight disputes, SPF shortfalls, parcels lost in transit and returns that came back with a different piece inside — filed as claims with the packing footage attached, and answered before they close. |
| Returns / RMA | `SPEC` | Customer returns, courier returns and wrong returns kept apart — because only one of the three is really your fault, and only one of them turns into dead stock. |
| Channels & Storefronts | `SPEC` | Connect a channel once and it stays in step: catalogue out, price out, stock out, orders in. |
| Labels & Documents | `SPEC` | The panel gives you a PDF; this hands the packing table something it can work from. |
...
| App | State | What it is |
|---|---|---|
| Staff & Contractors | `SPEC` | Attendance marked by tap, effective-dated salary, and karigar piece-rate earnings in a single register. |
| Time-off & Advances | `SPEC` | Leave, Diwali advances, and exactly how they change this month’s payout before you approve it. |
...
| Repricing Engine | `SPEC` | Rules per panel and per design — floor, ceiling, match-lowest and a festive override — so a Diwali sale does not quietly go below cost. |
| Automation | `SPEC` | If this happens, do that — across any module, without writing code. |
| Blog & Pages | `SPEC` | How to drape it, what to wear it to, which fabric for which season — written, scheduled and published to your own site with the meta and internal links already set. |
...
| App | State | What it is |
|---|---|---|
| Content Engine | `SPEC` | Fourteen stages in your own voice — buyer psychology, competitor reading, hooks, the product description, marketplace copy for Amazon and Myntra, ad variations, reel scripts, song lyrics for the reel, the calendar, size chart and alt text. |
| Image Studio | `SPEC` | A phone photo becomes a listing image: layers, free transform, background removal, Myntra 1080×1440 and every other channel preset, watermark and SEO alt text. |
| Video Studio | `SPEC` | Text and image to video, reels and ad cuts sized for every channel. |
| Design Studio | `SPEC` | Banners, festive creatives and thumbnails — templates, layers, undo and redo, any colour, exact sizing and stock elements, exporting PNG, JPG or PDF at whatever size the panel or the printer asks for. |
...
| Publisher | `SPEC` | One push sends the listing, images and copy to the website and every panel, and reports back what went live and what a panel rejected, with the reason. |
...
| App | State | What it is |
|---|---|---|
| Projects & Cases | `SPEC` | An exhibition, a custom order for a chain, a fit-out or a dispute — stages you define, owners, deadlines, documents, hours and real cost, all on one record the ledger can see. |
| Timesheets & Planning | `SPEC` | Who is on what this week and the hours that actually went in — against a project, an exhibition or a machine — with billable and non-billable kept apart. |
| Approvals | `SPEC` | One queue for everything waiting on a yes: a mill purchase order, a boutique discount, a leave day, a credit note, a payment. |
...
| Discuss | `SPEC` | The conversation attached to the record it is about — this order, this mill bill, this dispute — so a year later the reason for the decision is still sitting beside it. |
...
| App | State | What it is |
|---|---|---|
| CEO Dashboard | `WORKS TODAY` | Cash, sales by channel, stock by design, profit per piece and the alerts that matter — one screen, refreshed as the day runs. |
| Report Builder | `WORKS TODAY` | Drag the fields you want into a report and save it for the whole team. |
| Group Consolidation | `WORKS TODAY` | Ethnic Fashion, Vastrangam and Adini Couture as one set of figures, inter-company transfers removed, so the group position is real rather than three spreadsheets added together. |
...
| App | State | What it is |
|---|---|---|
| AI Assistant | `SPEC` | Ask in your own words — “what did Myntra actually pay us last week, and what is still short?”, “which designs did Kalamandir return most” — and get the answer with the rows it came from underneath it, each clicking through to the record. |
| AI Chatbot | `SPEC` | The same engine facing your buyer, on the website and on WhatsApp: where is my order, will the L fit me, I want to return this saree. |
| AI Agents | `SPEC` | A job rather than a question: “chase every unmatched payout line from last week and draft the claim for each.” It works out the steps and stops where a person has to decide. |
