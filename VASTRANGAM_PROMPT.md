...
Generated from this repository on 2026-09-17. Every count below is read from source and every path is checked to exist.
...
| `brand/site/rules.js` | **302 rules**, each with what the system will never do instead. |
| `brand/site/modules.js` | **31 modules · 165 apps** — the one canonical list. Read it; never type a count from it. |
...
| `MEDHAVA_PLAN_OF_ACTION.md` | WHAT gets built, in order, and all 302 rules. |
...
| `node brand/site/checkrules.js` | A rule claiming a proof it does not have — all 302 of them. |
