...
const DOCS = [
  ...
  /* ── THE MASTER-SPEC COVERAGE SHEET ───────────────────────────────────────
     ...
     The document below is the summary; the whole thing is MASTER_SPEC_COVERAGE.xlsx beside
     it, generated from the same register so the two cannot disagree. The spreadsheet is not
     in the delivery manifest because the manifest pairs a markdown with a rendered PDF, and
     a workbook is neither — it is regenerated by tools/masterspec_xlsx.py from the data this
     generator writes. */
  {
    md: 'MASTER_SPEC_COVERAGE.md', pdf: 'MASTER_SPEC_COVERAGE.pdf', edition: 'MEDHAVA',
    what: 'Every one of the 945 line items in the master specification, counted: what is covered, what is not, and what no amount of code can close — with the mapping to a real app resolved from the requirements registry rather than typed.',
    generator: 'node brand/delivery/website/mkmasterspec.js',
    decide: {
      modules: 'The specification is somebody else’s structure, not this product’s. Mapping its 945 lines onto 22 modules is exactly what the spreadsheet does row by row; repeating the module list here would be a second answer to a question the sheet already answers 945 times.',
      apps: 'Every line that maps names its app in full, so all 113 appear where they are relevant rather than in a list nobody reads. The count of apps that stand above SPECIFIED is stated, because that is the number the covered column depends on.',
      rules: 'The 293 rules govern behaviour once something runs. This counts how much runs at all, and a rule cannot be satisfied by an app that is only written down.',
      stack: 'Not one of the 945 lines is a choice of tool. Every one would sit in the same column whichever alternative in brand/site/stack.js were picked.',
      dynamic: 'Measured with no tenant installed, because the specification describes a product rather than one business’s configuration of it.',
  ...
  /* ── THE AUDIT-PHASE DOCUMENTS ────────────────────────────────────────────
     ...
     what is missing, and what to do on Monday — and each links to the others rather than
     repeating them. Four documents restating one measurement in longer words would be the
     thing §3 rule 3 forbids by name.

     THERE WERE FIVE. The fifth compared this product's coverage against another company's
     product list, and a comparison document is nothing but that company's name on every
     line. The owner asked for those names out of everything Medhava ships, so it is gone
     rather than reworded. The part of it that was genuinely about THIS product — the
     capability classes with no app at all — is Gap 7 of GAP_ANALYSIS.md, resolved from
     brand/site/backlog.js, which answers the same question from the owner's OWN
     specification instead of somebody else's catalogue.

     All four carry build state, like the roadmaps and unlike everything else here, and are
     gated by checkregistry.js and checkaudit.js rather than by the "describes a design"
     rule the other documents are held to. */
  {
...
const NOT_DELIVERED = {
  ...
  'HANDOFF.md':
    'The prompt a MODEL reads when it takes over this build from another one — Claude handing to ' +
    'Codex, Codex to Grok — so that work continues when a budget runs out. It is deliberately not ' +
    'a delivered document and deliberately has no PDF: nobody reads it on a phone, and rendering ' +
    'it would imply it is for the owner when its whole content is instructions for an agent. It ' +
    'is generated by mkhandoff.js, which refuses to write it if any path or npm script it names ' +
    'does not exist — a handover document that sends the next model to a missing file is worse ' +
    'than none, because the model then distrusts everything else in it.',
  ...
  'MEDHAVA_START_HERE.md':
    'The prompt the OWNER pastes into a fresh conversation to give a machine cold context — ' +
    'what Medhava and Vastrangam are, what is genuinely built, where the archives live, and how ' +
    'to talk to him. It is not delivered for the same reason HANDOFF.md is not: it is addressed ' +
    'to a model rather than to a reader, and rendering it as a PDF would put it in the reading ' +
    'pile it exists to sit in front of. HANDOFF.md is its sibling and starts one step later — ' +
    'that one opens with `npm ci` and assumes the repository is already in hand, which is the ' +
    'assumption this file exists to remove. Generated by mkstarthere.js, which refuses to write ' +
    'it if any path or command it names does not exist.',
