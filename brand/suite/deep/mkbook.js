...
const CFG = {
  ERP: { tag: 'ERP', company: 'Acme Corp (any industry)', edition: 'Unified ERP — any industry',
    ...
    mfg: 'Manufacturing / Planning', winner: 'Gamma Materials Co', outfile: 'Vanijo_Procurement_ERP.pdf' },
  VAS: { tag: 'VAS', company: 'Vastrangam', edition: 'Vastrangam — ethnic-wear D2C + marketplace',
    ...
    mfg: 'Manufacturing / Karigar', winner: 'Surat Cotton Mills', outfile: 'Vanijo_Procurement_Vastrangam.pdf' }
};

function page(inner, n) { return `<section class="pg"><div class="pgbody">${inner}</div><div class="foot"><span>Vanijo · Procurement — ${n.ed}</span><span>${n.p}</span></div></section>`; }
...
function book(c) {
  ...
  pages.push(`<section class="pg cover"><div class="cwrap">
    <div class="logo"><span class="mk">◑</span> Vanijo</div>
    <div class="ed">${ed}</div>
    <h1>Procurement</h1>
  ...
  pages.push(page(`<h2>Where Procurement sits</h2>
    <p>Every app in Vanijo sits on <b>one shared Data Core + event bus</b>: Item/SKU, Party (vendor), Stock, Ledger/Voucher, Order. Procurement <b>writes</b> to Stock (accepted goods), the Ledger (payable + tax credit) and Master Data (preferred vendors), and <b>reads</b> requirements from Manufacturing/Planning and reorder alerts from Inventory.</p>
...
RFQ AWARD   winner = min(rate) → creates approved PO at winning rate</pre>
  ...
  pages.push(page(`<h2>How to run &amp; accept</h2>
    ...
    <div class="end">Vanijo · Procurement — Module 1, App 1 of 6 · ${c.company}</div>`, P()));
...
.foot{position:absolute;left:17mm;right:17mm;bottom:8mm;display:flex;justify-content:space-between;font-size:9px;color:#8aa39c;border-top:1px solid #e3ede9;padding-top:5px}
...
.note{background:#eaf6f1;border-left:3px solid #0fae90;padding:9px 12px;border-radius:0 8px 8px 0;font-size:11.5px;color:#245}
...
.cover .lede{margin-top:20px;font-size:14px;line-height:1.7;color:#e6f5ef;max-width:150mm}
...
.core{background:#12312d;color:#fff;border:2px solid #0fae90;border-radius:10px;padding:12px 16px;text-align:center;margin-bottom:14px}
.core b{font-size:14px;letter-spacing:.03em}.core span{display:block;font-size:11px;color:#8fd7c6;margin-top:3px}
...
.end{margin-top:22px;text-align:center;font-size:12px;color:#8aa39c;letter-spacing:.02em}
`;
