'use strict';
/* Module 03 · Sales — five app books (× 2 editions) + the module book, all through the
...
/* ─── module book ─── */
function moduleBook() {
  const P = bookBuilder('Sales', 'Module 03');
  const pages = [];
  const T = k => (TESTS[k] || []).length;
  pages.push(`<section class="pg cover"><div class="cwrap">
    <div class="logo">${mark} Medhava</div><div class="ed">Module 03 of 16</div>
  ...
  pages.push(P(`<h2>What this module is</h2>
    <p class="big">Module 01 told you <b>what is happening</b>. Module 02 told you <b>who with</b>. Module 03 is where the money is actually recorded.</p>
...
for (const key of ['d2c', 'b2b', 'export', 'pos', 'quotes']) {
  ...
  for (const [ed, tag, cfgFile, edition, co] of [
    ...
    const c = Object.assign({}, s, {
      tag, edition, co, cfg,
      moduleLine: 'Module 03 · Sales — App ' + s.n + ' of 5',
...
jobs.push({ html: moduleBook(), out: 'Medhava_Module_03_Sales.pdf' });
render(jobs);
