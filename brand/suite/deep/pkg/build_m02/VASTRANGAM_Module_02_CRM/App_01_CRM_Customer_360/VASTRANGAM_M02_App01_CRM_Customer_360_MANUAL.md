...
WHAT YOU NEED
  ...
  About 61 KB — smaller than a photo from your phone.
...
AND AT THE BOTTOM
  ...
  the app after three months of use: if "Wanted a lower rate" keeps appearing, that
...
ADD A LEAD  (top panel, four boxes)
  ...
  Boutique / firm        The boutique, the wholesale firm, or the marketplace.
  Where did it come from  Boutique enquiry, referral, exhibition, marketplace category manager, Instagram DM.
...
THE OPEN DEALS TABLE
  Deal            Contact name, with the boutique or firm underneath.
...
THE CUSTOMER TABLE
  ...
  THE TABLE IS SORTED BY WORTH, NOT BY GROSS. This matters more in ethnic wear than almost anywhere. A marketplace account with a huge gross and 14% coming back can be worth less than a Jaipur boutique at 4% — and this ordering shows you that instead of hiding it.
...
WHAT THEY BUY, AND WHERE IT COMES BACK
  ...
  the return rate, and what they kept. This is the panel that settles arguments. If Flipkart is responsible for most of a buyer’s returns while Myntra is clean, the problem is that listing — not the buyer and not the fabric.

EVERY ORDER
  Order number, date, channel, amount, what was returned, and what was kept.
  Newest first. This is read from Channel Manager and Sales — never typed in here.
...
WHAT TO SAY TO EACH GROUP
  ...
  A boutique that reorders every six weeks is worth holding on to. A chain that has been silent since last Diwali needs one win-back offer tied to the festive window, and then letting go.
...
all READ from Channel Manager and Sales. They are never typed in here and never stored
here — which is exactly why the customer value on this screen can never
disagree with what Channel Manager and Sales says. There is only one copy of it.
