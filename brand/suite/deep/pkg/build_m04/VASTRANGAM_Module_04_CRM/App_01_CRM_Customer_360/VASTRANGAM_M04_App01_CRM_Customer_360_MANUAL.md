...
WHAT YOU NEED
  ...
  The whole app is ONE file: VASTRANGAM_M04_App01_CRM_Customer_360.html
  About 138 KB — smaller than a photo from your phone.


ON A WINDOWS COMPUTER
  1. Find the file VASTRANGAM_M04_App01_CRM_Customer_360.html — usually in your Downloads folder.
  ...
  2. Double-click VASTRANGAM_M04_App01_CRM_Customer_360.html.
...
ON A MAC
  1. Find VASTRANGAM_M04_App01_CRM_Customer_360.html in Downloads. If it came in a ZIP, double-click the ZIP
     to unpack it first.
  2. Double-click VASTRANGAM_M04_App01_CRM_Customer_360.html. Safari opens it.
  3. Done.
...
ON AN ANDROID PHONE OR TABLET
  1. Save VASTRANGAM_M04_App01_CRM_Customer_360.html to your phone. Any route works — WhatsApp, email,
     Google Drive, or a USB cable from your computer.
  2. Open the "Files" app (some phones call it "My Files").
  3. Go to Downloads and tap VASTRANGAM_M04_App01_CRM_Customer_360.html.
  ...
  MAKE IT LOOK AND FEEL LIKE A REAL APP:
     ...
     CRM & Customer 360 directly, with no browser bar. Exactly like any other app,
     and it still works with the phone in flight mode.


ON AN IPHONE OR IPAD
  1. Save VASTRANGAM_M04_App01_CRM_Customer_360.html to the "Files" app (Save to Files from WhatsApp, Mail
  ...
  MAKE IT LOOK AND FEEL LIKE A REAL APP:
     ...
     Now CRM & Customer 360 has its own icon on your home screen.
  ...
  WHAT "MARK WON" DOES
                ...
                tells you WHICH of the two things happened:
                  ...
                  "Won — attached to the record you already had"
                       That organisation was already a buyer. No
...
FIVE CARDS ACROSS THE TOP
  ...
  Customers worth What everyone you have won has bought, after returns.
                  ₹18,68,300.00.
  Needs a hand    Quiet buyers + open tickets + unsigned documents,
...
TWO PANELS BELOW
  ...
  What needs you — and this is the panel that proves the module is one
    module. It mixes THREE apps in one list:
        · quiet buyers          (from the customer records)
...
"EVERY OPEN DEAL" TABLE
  ...
  Each row has a "Mark won" button. Press it and:
     ...
     · the win rate moves
     · a buyer record appears on Customers — OR, if that
...
THE "SHOW" BUTTONS
  ...
  "Open →" on any row goes to that buyer's full record.

──────────────────────────────────────────────
SCREEN 4 · CUSTOMER 360    (the screen the whole module exists for)
──────────────────────────────────────────────
One buyer, everything, one page. As it ships this opens on
...
"WHAT TO DO NEXT"
  ...
  point is that the same buyer gets the same answer whoever opens
...
FOUR MORE PANELS
  Where they buy       — their orders split by channel, with returns per
                         channel. Flipkart at 14% returns on a big gross number can
                         easily earn less than the website at 11% on a
                         smaller one.
  On file for them     — every document filed against this buyer
...
──────────────────────────────────────────────
Nobody tags anybody. Every buyer lands in exactly one group, from
...
Because it is a rule, a buyer moves group BY THEMSELVES the moment
...
appear on that buyer's record. Agree them once, and everyone in
...
WHERE IS MY DATA KEPT?
  ...
  file, labelled "medhava_crm_vastrangam_v1".
...
MOVING FROM YOUR COMPUTER TO YOUR PHONE (or to a new machine)
  ...
  2. Send yourself both files — VASTRANGAM_M04_App01_CRM_Customer_360.html AND the backup.
  3. On the new device: open VASTRANGAM_M04_App01_CRM_Customer_360.html, then "Backup & Health" →
...
**File you need:** `VASTRANGAM_M04_App01_CRM_Customer_360.html` · opens by double-click · works offline · 42 self-tests
