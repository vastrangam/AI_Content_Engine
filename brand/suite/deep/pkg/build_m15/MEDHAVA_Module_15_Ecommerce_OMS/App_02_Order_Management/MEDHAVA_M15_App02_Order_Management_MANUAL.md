...
BEFORE ANY OF IT — THE ONE IDEA
  ...
  shown, from two things:
     ...
     2. HOW FAR THE WAREHOUSE IS.  Each warehouse has its own number of
        transit days to each zone. One warehouse is 1 day from a zone; another is 4 days from the same zone.
...
HOW THE CHANNELS DIFFER
  ...
  promises, return rate and on-time percentage. They behave nothing like each other, and one "orders" screen that treats them the same is how a business stops being able to tell which channel is working.
...
THE BUTTONS, IN THE ORDER THEY APPEAR
  ...
  from. A DAMAGED ONE DOES NOT. A damaged piece added back to stock is phantom inventory: it will be promised to somebody, allocated, picked, and then not be there.
...
WHERE IS MY DATA KEPT?
  ...
  file, labelled "medhava_ord_erp_v1".
