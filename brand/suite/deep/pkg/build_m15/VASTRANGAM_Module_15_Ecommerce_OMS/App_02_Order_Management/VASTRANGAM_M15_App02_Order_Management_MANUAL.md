# Order Management — Complete Manual
### Medhava · Vastrangam — ethnic-wear D2C + marketplace
**Module 04 · E-commerce / OMS — App 2 of 2**
...
════════════════════════════════════════════════════════════════════════
   ...
   M E D H A V A  ·  ORDER MANAGEMENT
...
For Vastrangam that means three warehouses — Delhi, Mumbai and Bengaluru —
...
arithmetic 55 different ways every time it starts.
...
WHAT YOU NEED
  ...
  The whole app is ONE file: Order_Management.html
  About 103 KB — smaller than a photo from your phone.


ON A WINDOWS COMPUTER
  1. Find the file Order_Management.html — usually in your Downloads folder.
  ...
  2. Double-click Order_Management.html.
...
ON A MAC
  1. Find Order_Management.html in Downloads. If it came in a ZIP, double-click the ZIP
     to unpack it first.
  2. Double-click Order_Management.html. Safari opens it.
  3. Done.
...
ON AN ANDROID PHONE OR TABLET
  1. Save Order_Management.html to your phone. Any route works — WhatsApp, email,
     Google Drive, or a USB cable from your computer.
  2. Open the "Files" app (some phones call it "My Files").
  3. Go to Downloads and tap Order_Management.html.
  ...
  MAKE IT LOOK AND FEEL LIKE A REAL APP:
     ...
     Order Management directly, with no browser bar. Exactly like any other app,
     and it still works with the phone in flight mode.


ON AN IPHONE OR IPAD
  1. Save Order_Management.html to the "Files" app (Save to Files from WhatsApp, Mail
  ...
  MAKE IT LOOK AND FEEL LIKE A REAL APP:
     ...
     Now Order Management has its own icon on your home screen.
...
════════════════════════════════════════════════════════════════════════
  ...
  LEFT MENU     Seven screens in four groups:
...
BEFORE ANY OF IT — THE ONE IDEA
  ...
  shown, from two things:
     ...
     1. WHEN IT DISPATCHES.  Orders placed before the 14:00 cut-off go
        out the same day. After it, the next day.
     2. HOW FAR THE WAREHOUSE IS.  Each warehouse has its own number of
        transit days to each zone. Bengaluru is 1 day from Hyderabad; Delhi is 4 days from it.
...
HOW THE CHANNELS DIFFER
  ...
  promises, return rate and on-time percentage. A showroom sale is finished the moment it is paid for; a Myntra order comes back one time in four; a boutique order is a credit decision before it is a fulfilment one.
...
ONE PANEL PER WAITING ORDER
  ...
  An order nothing can serve needs a purchase or a production order, not a promise — and nothing is gained by giving the customer a date in the meantime.
...
THE BUTTONS, IN THE ORDER THEY APPEAR
  ...
  3. "Pay the refund"     Full amount if resaleable; 50% if damaged.

  A RESALEABLE PIECE GOES BACK INTO STOCK at the warehouse it shipped
  from. A DAMAGED ONE DOES NOT. A saree back with a pulled zari or a make-up mark on the pallu is not resaleable at ₹4,999, and adding it back at full value is how a healthy-looking inventory turns out to be worth half of what the report says.
...
WHERE IS MY DATA KEPT?
  ...
  file, labelled "medhava_ord_vastrangam_v1".
...
MOVING FROM YOUR COMPUTER TO YOUR PHONE (or to a new machine)
  ...
  2. Send yourself both files — Order_Management.html AND the backup.
  3. On the new device: open Order_Management.html, then "Backup & Health" →
...
Every time Order Management opens, it quietly checks its own arithmetic — 55
separate checks — before showing you anything. You can see the result:

  Click "Backup & Health" → look at the "Self-tests" panel.

You should see 55/55 pass. Each line is written in plain
language, so you can read what was actually checked. For example:

  · "no promise date is stored anywhere — it is worked out on every read"
...
**File you need:** `Order_Management.html` · opens by double-click · works offline · 55 self-tests
**Companion app in this module:** Marketplace OMS
