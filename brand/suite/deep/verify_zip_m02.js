'use strict';
/* The Module 02 ZIPs, opened the way a customer opens them.
...
(async () => {
  ...
  for (const { ed, key } of EDS) {
    const base = path.join(ROOT, `${ed}_Module_02_CRM`);
    console.log('\n══ ' + ed);
    check('the edition folder is there', fs.existsSync(base), base);

    for (const f of [`${ed}_M02_Module_Overview.pdf`, `${ed}_M02_START_HERE.md`]) {
    ...
    for (const a of APPS) {
      ...
      const stem = `${ed}_M02_App${a.n}_${a.slug}`;
