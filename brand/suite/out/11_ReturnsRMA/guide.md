...
> Return requests, reasons & refunds — close the loop from RMA to restock.
...
## What works
A returns desk. Each RMA flows requested → approved → received → refunded; refund value and reason mix are computed, and received items restock. Return rate is against a demo order base.
