...
check('a future-dated raise activates by itself when that month arrives', () => {
  const log = new EffectiveLog('salary');
  log.setValue('varunkotecha', '2025-04-01', 9000);
  log.setValue('varunkotecha', '2026-08-01', 10000);
  assert.strictEqual(log.resolve('varunkotecha', '2026-07'), 9000);
  assert.strictEqual(log.resolve('varunkotecha', '2026-08'), 10000);
});
...
function seed() {
  ...
  db.insert('designs', { id: 'muspur', company_id: 'vs', design_code: 'MUSPUR', design_name: 'varunkotechaPurple Anarkali', set_type: 'Anarkali Plazo Set', status: 'active', created_at: now });
