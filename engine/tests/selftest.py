...
def test_names():
    ...
    a = AliasTable()
    a.register("farhanamin", "farhanamin", "FARHAN AMIN", "farhanamin")
    check("every written form of a name resolves to one id",
          a.lookup("FARHAN AMIN") == a.lookup("  farhanamin ") == "farhanamin")
...
def test_no_uncited_piece_rate():
    ...
    planted["piece_rate"].append({"key": "ishitasompura", "from": "2025-04-01", "to": None,
                                  "value": {"rate": 100, "unit": "per_hour"}})
    check("and planting an uncited rate is caught",
          any(p.startswith("ishitasompura:") for p in uncited_piece_rates(planted)),
...
EXPECTED_BLENDED = {
    "devdave": 164.43, "rohanchavda": 63.49, "varunkotecha": 40.18, "farhanamin": 82.14,
    "kabirpatel": 160.71, "chetnavora": 117.86, "sanyabhatt": 53.57, "eshasolanki": 53.57,
    "aaravmehta": 37.95, "tanvigandhi": 35.71,
}
...
CORPUS_EXPECTED = {
    ...
    # 9,75,649 is the days-based half only: it leaves out meerachauhan, whose 518
